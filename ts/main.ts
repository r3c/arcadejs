...
let state = {
	...
	screenSize: { x: screen.width, y: screen.height }
};
...
let display = function (state: State) {
	...
	context.fillRect(0, 0, state.screenSize.x, state.screenSize.y);
	...
	for (let face of faces) {
		let point1 = scene.perspective(vertices[face[0]], state.screenSize);
		let point2 = scene.perspective(vertices[face[1]], state.screenSize);
		let point3 = scene.perspective(vertices[face[2]], state.screenSize);
		let point4 = scene.perspective(vertices[face[3]], state.screenSize);
