...
const gravityIncrement: Vector3 = Vector3.fromCustom((vector) => {
  vector.set(belowIncrement);
  vector.scale(0.02);
});
...
const createWorldGraphic = (
  ...
  const shift = Vector3.fromCustom((vector) => {
    vector.set(renderSize);
    vector.scale(-0.5);
  });
  ...
  const worldGraphic: WorldGraphic = {
    ...
    setVoxel: (offset, modelIndex) => {
      ...
      if (modelIndex !== undefined) {
        ...
        const transform = Matrix4.fromCustom((matrix) => {
    ...
    tick: () => {
      ...
      for (const activeIndex of activeIndices) {
        ...
        if (move === undefined) {
          ...
          const nextOffset = Vector3.fromCustom((vector) => {
            vector.set(activeOffset);
            vector.add(move);
          });
          ...
          if (isValid(nextOffset) && blocks[nextIndex] === undefined) {
            ...
            const aboveOffset = Vector3.fromCustom((vector) => {
              vector.set(activeOffset);
              vector.add(aboveIncrement);
            });
            ...
            const belowOffset = Vector3.fromCustom((vector) => {
              vector.set(activeOffset);
              vector.add(belowIncrement);
            });
