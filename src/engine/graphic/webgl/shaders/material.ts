...
const materialType = createSingletonFunction<[]>(
  shader`\
...
${materialType()} materialSample(
  ...
  vec4 diffuseLinear = vec4(${standardToLinear("diffuseSample.rgb")}, diffuseSample.a);
  vec4 combinedDiffuseColor = diffuseColor * diffuseLinear;

  vec4 specularSample = texture(specularMap, coordinate);
  vec4 specularLinear = vec4(${standardToLinear("specularSample.rgb")}, specularSample.a);
