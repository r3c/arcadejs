...
vec3 pbrGetLight(in vec3 normal, in vec3 eyeDirection, in vec3 lightDirection, in vec3 lightColor, in vec3 albedo, in float roughness, in float metalness) {
    const float f0 = 0.04;

    vec3 albedoColor = albedo * (1.0 - f0) * (1.0 - metalness);
	vec3 specularColor = mix(vec3(f0), albedo, metalness);
    ...
    vec3 diffuseContrib = (1.0 - F) * albedoColor / PBR_PI;
...
const lightInvoke = (normal: string, eyeDirection: string, lightDirection: string, lightColor: string, albedo: string, roughness: string, metalness: string) =>
    `pbrGetLight(${normal}, ${eyeDirection}, ${lightDirection}, ${lightColor}, ${albedo}, ${roughness}, ${metalness})`;
